...
namespace BasicWebApi.ReinforcedTypings
{
    public static class ReinforcedTypingsConfiguration
    {
        ...
        {
            var types = Assembly.GetExecutingAssembly().GetTypes().Where(_ => _.Namespace.StartsWith(typeof(ExampleDto).Namespace)); ;
            var classTypes = types.Where(_ => _.IsClass);
            var enumTypes = types.Where(_ => _.IsEnum);

            builder.Global(_ => _.UnresolvedToUnknown());

            builder.ExportAsInterfaces(classTypes, _ => _.AutoI(false).WithPublicProperties());
